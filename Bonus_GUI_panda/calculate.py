...
def cal(gender,height,weight,age):
    ...
    return (
        "%5.2f"%bmi.value(height,weight),
        bmi.type(bmi.value(height,weight)),
        "%5.2f至%5.2f"%(18.5*(int(height)/100)**2,24*(int(height)/100)**2),
        "%5.2f"%bmr.value(height,weight,gender,age),
        bmr.type(bmr.value(height,weight,gender,age),gender)
    )
